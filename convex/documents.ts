...
export const restore = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const userId = identity.subject;
        const exitstingDocument = await ctx.db.get(args.id);
        if (!exitstingDocument) {
            throw new Error("Document not found");
        }
        if (exitstingDocument.userId !== userId) {
            throw new Error("Not authorized");
        }
        ...
        if (exitstingDocument.parentDocument) {
            const parent = await ctx.db.get(exitstingDocument.parentDocument);
...
export const update = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const userId = identity.subject;
        const { id, ...rest } = args;
        const exitsingDocument = await ctx.db.get(id);
        if (!exitsingDocument) {
            throw new Error("Not found");
        }
        if (exitsingDocument.userId !== userId) {
